...
DATASET_PATH = Path(PROJECT_PATH, "data/dataset_main")
RAW_DATA_PATH = Path(DATA_PATH, "raw_data_bgr")
FRAMES_PATH = Path(DATA_PATH, "frame_data")
...
def get_frames(filename, n_frames=1):
    ...
    if length >= target_length:
        new_frames = frames[:target_length]
    elif length < target_length:
        new_frames = frames + [
            np.zeros_like(frame) for _ in range(0, target_length - length)
        ]

    return new_frames, v_len
    # return new_frames[::3], v_len
